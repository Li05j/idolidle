<script lang="ts">
    ...
    let progress_percent = $derived(Math.min((checkpoints.current_time_spent / checkpoints.current_total_time) * 100, 100));
...
<div class="bg-white p-6 rounded-lg shadow-md mb-4 top-0 left-0 w-full h-36">
    <div class="flex justify-between items-center mb-4">
        ...
        <div class="text-gray-600 text-sm">
            {msToSecF(checkpoints.current_time_spent)}/{msToSecF(checkpoints.current_total_time)}s
        </div>
    </div>
