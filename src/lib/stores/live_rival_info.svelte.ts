...
class PlayerRivalCompare {
    ...
    private spd_ratio = $derived(stat_list.Speed.final / RivalStatsM.get_stats(CPs.current_completed_checkpoint).Speed)
    public spd_clamped = $derived(Math.max(0, Math.min(1, this.spd_ratio / 2))) // within [0,1]
    public spd_color = $derived(`background-color: hsl(${this.spd_clamped * HUE_CONST}, 100%, 50%)`)
