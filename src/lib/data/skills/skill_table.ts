...
export const ALL_SKILLS: SkillDef[] = [
    {
        ...
        chance: 0.5,
        values: { threshold: 0.25, restore: 0.15 },
    ...
    },
    {
        ...
        eff_string: (v) => `{Self_poss} next move poaches ${fmt_pct(v.buff - 1)} more Fans.`,
        ...
        effect: ({ you, apply_temp_buff, log, values: v }) => {
            ...
            log(`Underdog: {Self} got fired up! {Self_poss} next move is ${fmt_pct(v.buff - 1)} more effective!`);
        },
    },
    {
        id: 'idol_executive',
    ...
    },
    {
        ...
        condition: () => true,
        effect: ({ set_dmg_reduction, on_after_attack, log, values: v }) => {
            set_dmg_reduction?.(v.reduction);
            on_after_attack?.((fans_stolen) => {
                if (fans_stolen > 0) {
                    const saved = Math.round((fans_stolen / (1 - v.reduction)) * v.reduction);
                    log(`Good Student: {Self} stood unwavering. ${saved} Fans ended up not leaving!`);
                } else {
                    log(`Good Student: {Self} stood unwavering. No Fans were lost!`);
                }
            });
        },
    },
    ...
    },
    {
        ...
        eff_string: (v) => `Increase Charm by ${fmt_pct(v.charm_buff - 1)} for 1 turn. If {opp} fails to poach Fans, {opp} loses ${fmt_pct(v.penalty)} Fans.`,
        ...
        effect: ({ you, rival, apply_temp_buff, on_after_attack, log, values: v }) => {
            ...
            log(`Moe Kyun~!: {Self_poss} Charm soars by ${Math.round(you.Charm * (v.charm_buff - 1))}!`);
            on_after_attack!((fans_stolen) => {
                if (fans_stolen <= 0) {
                    const drained = drain_fans(rival, you, rival.Fans * v.penalty);
                    log(`Moe Kyun~!: {Opp_poss} attempt was BOO'd instead! ${drained} Fans took your side!`);
                }
            });
        },
    },
    {
        ...
        condition: ({ you, values: v }) => you.Curr_Stamina < you.Max_Stamina * v.threshold,
        effect: ({ you, log, values: v }) => {
            const sing_before = you.Sing;
            const dance_before = you.Dance;
            you.Sing *= v.buff;
            you.Dance *= v.buff;
            log(`Final Burst: {Self} is preparing to all in! Sing increased by ${Math.round(you.Sing - sing_before)} and Dance by ${Math.round(you.Dance - dance_before)} for the rest of the LIVE!`);
        },
    },
    {
        id: 'old_fashioned',
        ...
        effect: ({ rival, log, values: v }) => {
            const before = rival.Haste;
            rival.Haste *= v.haste_debuff;
            log(`Old Fashioned: {Self} lectures {Opp} for flexing too much, {Opp_poss} Haste dropped by ${Math.round(before - rival.Haste)} for this LIVE!`);
        },
    },
    {
        id: 'seasoned_runner',
        ...
        effect: ({ you, apply_temp_buff, log, values: v }) => {
            ...
            log(`Seasoned Runner: {Self} is ready to take the lead! Haste increased by ${Math.round(you.Haste * (v.haste_buff - 1))}!`);
        },
    },
    {
        id: 'showstopper',
        ...
        triggers: ['before_inflicting_dmg'],
        chance: 0.25,
        values: { stam_cost: 0.15, buff: 2.25 },
        ...
        effect: ({ you, apply_temp_buff, log, values: v }) => {
            const stam_used = Math.round(you.Max_Stamina * v.stam_cost);
            you.Curr_Stamina -= you.Max_Stamina * v.stam_cost;
            apply_temp_buff!('you', 'Sing', you.Sing * v.buff);
            apply_temp_buff!('you', 'Dance', you.Dance * v.buff);
            log(`Showstopper: {Self} pulls out all the stops!! Burned ${stam_used} Stamina to make {Self_poss} next move ${v.buff}x more effective!`);
        },
    },
    {
        id: 'scandal_magnet',
        ...
        chance: 0.35,
        values: { reflect: 0.5 },
        cond_string: '{Opp} poaches Fans with a Sing move',
        eff_string: (v) => `Poach back ${fmt_pct(v.reflect)} of the Fans lost.`,
        ...
        effect: ({ you, rival, log, fans_stolen, values: v }) => {
            ...
            log(`Scandal Magnet: Scandalous! {Self} dissed {Opp} and poached ${taken} Fans back from {Opp}!`);
        },
    },
    {
        id: 'lullaby',
        ...
        effect: ({ rival, apply_temp_buff, log, values: v }) => {
            ...
            log(`Lullaby: {Self} soothing voice lowered {Opp_poss} Charm by ${Math.round(rival.Charm * (1 - v.charm_debuff))}!`);
        },
    },
    {
    ...
    },
    {
        ...
        eff_string: (v) => `Poaches an additional Charm × ${fmt_pct(v.extra_charm_dmg)} Fans.`,
        condition: () => true,
        effect: ({ you, rival, log, values: v }) => {
